...
angular.module('amChartsDirective', []).directive('amChart', ['$q', function ($q) {
  return {
    ...
    link: function ($scope, $el) {
      ...
      var mydata = [];
      var str = "";
      if (typeof $scope.tradeavgsneto !== 'undefined') { //also special setup
        // console.log($scope.tradeavgsneto);
        // console.log($scope.maintitle);
        // console.log($scope.type);
        mydata = [];
        for (var i=0; i<$scope.tradeavgsneto.length; i++) {
          mydata.push({title: (i+1), yield: $scope.tradeavgsneto[i] });
        }
        // console.log(mydata);
          $scope.options = {
            titles: [{
            ...
            chartCursor: {categoryBalloonEnabled: false, enabled: false}
        };
      }
      if (typeof $scope.bestaccuyields !== 'undefined') { //also special setup
        mydata = [];
        mydata.push({title:$scope.firsttitle + str, yield: $scope.firstvalue.toFixed(1)});
        for (var i=0; i<$scope.bestaccuyields.length; i++) {
          mydata.push({title: (i+1), yield: $scope.bestaccuyields[i] });
        }
        // console.log(mydata);
          $scope.options = {
            titles: [{
      ...
      if (typeof $scope.avgdayspertrade !== 'undefined') { //also special setup
        mydata = [];
        for (var i=0; i<$scope.avgdayspertrade.length; i++) {
          mydata.push({title: i+1, yield: $scope.avgdayspertrade[i] });
        }
          $scope.options = {
            titles: [{
                size: 9,
      ...
      if (typeof $scope.secondtitle !== 'undefined') {
        //$scope.options = JSON.parse('{"titles": [{"size": 12, "text": "' + $scope.options.maintitle + '"}], "data": [{"title": "' + $scope.options.firsttitle + '", "yield": ' + $scope.options.firstvalue + '}, {"title": "' + $scope.options.secondtitle + '", "yield": ' + $scope.options.secondvalue + '}], "type": "' + $scope.options.type + '", "theme": "light", "graphs": [{"type": "column", "title": "תשואה", "valueField": "yield", "fillAlphas": 1 }] }');
          $scope.options = {
            titles: [{
                size: 10,
                text: $scope.maintitle
            ...
            data: [{
                title: $scope.firsttitle,
                yield: $scope.firstvalue,
            }, {
                title: $scope.secondtitle,
                yield: $scope.secondvalue
            }],
            type: $scope.type,
            theme: "light",
